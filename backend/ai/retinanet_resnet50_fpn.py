...
PRE_TRAINED_BOOL = True):
    ...
    model = torchvision.models.detection.retinanet_resnet50_fpn(pretrained=PRE_TRAINED_BOOL)
    in_features_anchor = model.head.classification_head.conv[0].out_channels
    num_anchors = model.head.classification_head.num_anchors
    model.head = RetinaNetHead(in_features_anchor, num_anchors, num_classes)
    ...
    def evaluate(model, data_loader, device):
        ...
        plt.ylabel('F1 Score')
        plt.legend()
        plt.savefig(f'retinanet_resnet50_fpn_{PRE_TRAINED_BOOL}_{EPOCHS}_{VAL_IMAGES.replace("/", "-").replace(".", "")}_f1_over_confidence')
        ...
        plt.legend()
        plt.savefig(f'retinanet_resnet50_fpn_{PRE_TRAINED_BOOL}_{EPOCHS}_{VAL_IMAGES.replace("/", "-").replace(".", "")}_precision_over_recall')
        ...
        plt.ylabel('Precision')
        plt.legend()
        plt.savefig(f'retinanet_resnet50_fpn_{PRE_TRAINED_BOOL}_{EPOCHS}_{VAL_IMAGES.replace("/", "-").replace(".", "")}_precision_over_confidence')
        ...
        plt.ylabel('Recall')
        plt.legend()
        plt.savefig(f'retinanet_resnet50_fpn_{PRE_TRAINED_BOOL}_{EPOCHS}_{VAL_IMAGES.replace("/", "-").replace(".", "")}_recall_over_confidence')
